...
#define ETH_INTERFACE_MININET "meth"
#define ETH_INTERFACE_MININET_WEST "client-eth"  /* MUST match lab2/netinfo/links.csv */
#define ETH_INTERFACE_MININET_EAST "east"        /* MUST match lab2/netinfo/links.csv */
 ...
 */
in_addr_t ip_from_self(bool is_mininet) {
  struct ifaddrs *addrs = NULL, *iface;
  in_addr_t ip_addr = 0;
  getifaddrs(&addrs);

  /* Need to find the IP address from the correct interface. */
  /* char *correct_iface = (is_mininet) ? ETH_INTERFACE_MININET : ETH_INTERFACE; */
  char check_iface[100];
  char check_iface_full[100];
  memset(check_iface, 0, 100);
  memset(check_iface_full, 0, 100);
  ...
  for (iface = addrs; iface != NULL; iface = iface->ifa_next) {
    if (iface->ifa_addr && iface->ifa_addr->sa_family == AF_INET) {
      ...
      memcpy(check_iface_full, iface->ifa_name, strlen(iface->ifa_name));

      if (!is_mininet) {
        if (strcmp(check_iface, ETH_INTERFACE) == 0)
          goto found;
      } else {
        if (strcmp(check_iface, ETH_INTERFACE_MININET) == 0)
          goto found;
        if (strcmp(check_iface_full, ETH_INTERFACE_MININET_EAST) == 0)
          goto found;
        if (strcmp(check_iface, ETH_INTERFACE_MININET_WEST) == 0)
          goto found;
      }
    }
  }
...
found:
  ...
  freeifaddrs(addrs);
  fprintf(stderr, "[INFO] Found address 0x%x\n", (int) ip_addr);
  return ip_addr;
}
